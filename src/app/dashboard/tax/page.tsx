...
export default function TaxPage() {
  ...
  const handleDownloadPDF = () => {
    ...
    if (printWindow) {
      ...
      // Trigger print dialog (Save as PDF) once content loads
      printWindow.onload = () => printWindow.print();
      // Fallback if onload doesn't fire (inline HTML)
  ...
  const handleSendToCPA = () => {
    ...
    const body = encodeURIComponent(
      `Hi,\n\nPlease find attached the Schedule E expense report for tax year ${selectedYear}, generated by HostFi.\n\nThe CSV file contains per-property expense breakdowns mapped to IRS Schedule E line items.\n\nPlease review at your convenience.\n\nBest regards`
    );
    const mailto = cpaEmail ? `mailto:${cpaEmail}` : 'mailto:';
          ...
          <div className="bg-white rounded-2xl border border-gray-200 w-full max-w-md p-6 space-y-5" onClick={e => e.stopPropagation()}>
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                We&apos;ll download the CSV report and open your email client with a pre-filled message.
              </p>
            </div>
            ...
            {!cpaSent ? (
              <>
                <div>
                  <label htmlFor="cpa-email" className="block text-sm font-medium text-gray-700 mb-1.5">CPA Email (Optional)</label>
                  <input
                    id="cpa-email"
                    type="email"
                    value={cpaEmail}
                    ...
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 text-sm placeholder:text-gray-400"
                  />
                  <p className="text-xs text-gray-400 mt-1.5">Leave blank to choose the recipient in your email client.</p>
                </div>
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="flex-1 py-2.5 text-sm font-semibold text-white bg-gray-900 rounded-xl hover:bg-gray-800 transition-colors flex items-center justify-center gap-2"
                  >
                    <Share2 className="w-4 h-4" /> Download & Email
                  </button>
                </div>
              </>
            ) : (
              <div className="text-center space-y-4">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-900">CSV Downloaded & Email Opened</p>
                  <p className="text-xs text-gray-500 mt-1">Attach the downloaded CSV file to the email before sending.</p>
                </div>
                <button
                  onClick={() => setShowCPAModal(false)}
                  className="w-full py-2.5 text-sm font-medium text-gray-700 bg-gray-100 rounded-xl hover:bg-gray-200 transition-colors"
                >
                  Done
                </button>
              </div>
