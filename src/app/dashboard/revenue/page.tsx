...
export default function RevenuePage() {
  ...
  // P&L Export functions
  const handlePnLCSV = useCallback(() => {
    const rows = [
      ['Property', 'Bookings', 'Gross Revenue', 'Platform Fees', 'Net Payouts', 'Expenses', 'Profit', 'Margin %'],
      ...propertyPnL.map(row => [
        row.property.name,
        row.bookings,
        row.gross.toFixed(2),
        ...
        row.net > 0 ? ((row.profit / row.net) * 100).toFixed(1) : '0.0',
      ]),
      [],
      ['TOTAL', propertyPnL.reduce((s, r) => s + r.bookings, 0), totalGross.toFixed(2), totalFees.toFixed(2), totalNet.toFixed(2), totalExpenses.toFixed(2), netProfit.toFixed(2), totalNet > 0 ? ((netProfit / totalNet) * 100).toFixed(1) : '0.0'],
    ];
    const csv = rows.map(r => Array.isArray(r) && r.length > 0 ? r.map(c => `"${c}"`).join(',') : '').join('\n');
    const now = new Date();
    const dateStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
    downloadFile(csv, `hostfi-pnl-${dateStr}.csv`, 'text/csv');
  }, [propertyPnL, totalGross, totalFees, totalNet, totalExpenses, netProfit]);

  const handlePnLPDF = useCallback(() => {
    const html = `<!DOCTYPE html><html><head><title>HostFi P&L Report</title><style>
      body{font-family:-apple-system,BlinkMacSystemFont,sans-serif;padding:40px;color:#111}
      h1{font-size:20px;margin-bottom:4px} .sub{color:#666;font-size:13px;margin-bottom:24px}
      table{width:100%;border-collapse:collapse;font-size:13px}
      th{text-align:left;padding:8px 12px;border-bottom:2px solid #e5e7eb;font-size:11px;text-transform:uppercase;color:#666;letter-spacing:0.05em}
      td{padding:8px 12px;border-bottom:1px solid #f3f4f6}
      .right{text-align:right} .bold{font-weight:600}
      .green{color:#0d9488} .red{color:#e11d48}
      .total{background:#f9fafb;font-weight:600;border-top:2px solid #e5e7eb}
      .footer{margin-top:32px;font-size:11px;color:#999}
    </style></head><body>
      <h1>Profit & Loss by Property</h1>
      <div class="sub">Generated by HostFi — ${new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</div>
      <table>
        <thead><tr>
          <th>Property</th><th class="right">Bookings</th><th class="right">Gross Revenue</th>
          <th class="right">Platform Fees</th><th class="right">Net Payouts</th>
          <th class="right">Expenses</th><th class="right">Profit</th><th class="right">Margin</th>
        </tr></thead>
        <tbody>
          ${propertyPnL.map(row => {
            const margin = row.net > 0 ? ((row.profit / row.net) * 100) : 0;
            const profitClass = row.profit >= 0 ? 'green' : 'red';
            return `<tr>
              <td class="bold">${row.property.name}</td>
              <td class="right">${row.bookings}</td>
              <td class="right">${formatCurrency(row.gross)}</td>
              <td class="right">-${formatCurrency(row.gross - row.net)}</td>
              <td class="right">${formatCurrency(row.net)}</td>
              <td class="right">-${formatCurrency(row.expenses)}</td>
              <td class="right ${profitClass}">${row.profit >= 0 ? '' : '-'}${formatCurrency(Math.abs(row.profit))}</td>
              <td class="right ${profitClass}">${margin.toFixed(1)}%</td>
            </tr>`;
          }).join('')}
          <tr class="total">
            <td>Total</td>
            <td class="right">${propertyPnL.reduce((s, r) => s + r.bookings, 0)}</td>
            <td class="right">${formatCurrency(totalGross)}</td>
            <td class="right">-${formatCurrency(totalFees)}</td>
            <td class="right">${formatCurrency(totalNet)}</td>
            <td class="right">-${formatCurrency(totalExpenses)}</td>
            <td class="right ${netProfit >= 0 ? 'green' : 'red'}">${netProfit >= 0 ? '' : '-'}${formatCurrency(Math.abs(netProfit))}</td>
            <td class="right ${netProfit >= 0 ? 'green' : 'red'}">${totalNet > 0 ? ((netProfit / totalNet) * 100).toFixed(1) : '0.0'}%</td>
          </tr>
        </tbody>
      </table>
      <div class="footer">hostfi.ai</div>
    </body></html>`;
